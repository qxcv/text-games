...
class MachineOfDeathSimulator:
    ...
    def __init__(self, doShuffle, doParaphrase=False):
        ...
        if self.doParaphrase:
            ...
            self.dict_paraphrase = {
                action_orig: action_para
    ...
    def Read(self):
        ...
        if self.doParaphrase:
            self.actions = [
                self.dict_paraphrase[action]
                if action in self.dict_paraphrase
                else action
                for action in self.actions
            ]
        return (
            self.text,
            [self.actions[i] for i in self.idxShuffle]
            if self.doShuffle
            else self.actions,
            AssignReward(self.text, "machineofdeath"),
        )
    ...
    def tiddler8(self):
        ...
        if self.params["firstDeathGiven"] == 1:
            if (
                self.params["oldAge"] != 0
                and self.params["lookingUp"] != 0
                and self.params["jovi"] == 0
            ):
    ...
    def tiddler37(self):
        ...
        if self.params["win"] == 1:
            self.text += (
                """ (currently being safely guarded by your Wee Rex Adorable Plush"""
            )
    ...
    def tiddler50(self):
        ...
        self.actions += [
            "\xe2\x80\x9cWhat do you see in a place like that?\xe2\x80\x9d",
            "\xe2\x80\x9cIs this how all your nights end up?\xe2\x80\x9d",
        ]
        return
    ...
    def tiddler76(self):
        ...
        if (
            ...
            and self.params["seePoison"] == 1
            and self.params["manWait"] < 1
        ):
    ...
    def tiddler78(self):
        ...
        self.actions += [
            "&quot;Maybe you're right.\xe2\x80\x9d",
            "&quot;I'll be fine.\xe2\x80\x9d",
        ]
        return
    ...
    def tiddler103(self):
        ...
        if self.params["timeToGo"] < 0:
            self.text += (
                """You're now """ + str(self.params["timeToGo"]) + """ minutes late!"""
            )
    ...
    def tiddler135(self):
        ...
        if self.params["timeToGo"] > 0:
            self.text += (
                """You have """
                + str(self.params["timeToGo"])
                + """ minutes left after this debacle!"""
            )
        if self.params["timeToGo"] == 0:
            self.text += """You're about to be late!"""
        if self.params["timeToGo"] < 0:
    ...
    def tiddler153(self):
        ...
        if self.params["showered"] == 1:
            ...
            self.text += (
                """You didn't shower, and cursed the room with a pungent funk.\n"""
            )
        if self.params["brushedTeeth"] == 1:
            self.text += (
                """You brushed your teeth, and showed off your winning smile.\n"""
            )
            self.params["interviewScore"] = self.params["interviewScore"] + 1
        else:
        ...
        if self.params["dressedWell"] == 1:
            ...
            self.text += (
                """The panel looked at your messy outfit with disapproving eyes.\n"""
            )
    ...
    def tiddler183(self):
        ...
        if self.params["coins"] > 0:
            self.text += (
                """&lt;html&gt;&lt;/br&gt;&lt;/html&gt;Maybe today is the day. \n"""
            )
...
if __name__ == "__main__":
    ...
    while numEpisode < 10:
        ...
        print("TEXT:", text)
        print("REWARD:", reward)
        print("ACTIONS:", ", ".join(f"({idx}) {a}" for idx, a in enumerate(actions)))
        if len(actions) == 0 or numStep > 250:
            ...
            while True:
                try:
                    playerInput = int(input(f"Choose an action (0-{len(actions)-1}): "))
                    break
                except ValueError:
                    print(
